...
if ( parsed.error & 1 && 'log' !== Program.dispatcher ) {
	exit_error = true;
...
if ( parsed.error & 16 ) {
	exit_error = true;
	process.stderr.write( chalk.red( 'You must configure a `' + chalk.yellow( '.lansarerc' ) + '` file in the project root.' ) + os.EOL );
}
if ( parsed.error & 32 ) {
	exit_error = true;
	process.stderr.write( chalk.red( 'Please specify an environment as configured in the `' + chalk.yellow( '.lansarerc' ) + '` file.' ) + os.EOL );
}
