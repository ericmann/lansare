...
function notify( config ) {
	...
	return new Promise( function( fulfill, reject ) {
		...
		// Date | User | App Name | App ID | Description | Changelog | Revision
		var components = [];
		components.push( new Date().toString() );
		components.push( config.user );
		components.push( config.name ? config.name : 'unknown' );
		components.push( config.nrid ? config.nrid : 'unknown' );
