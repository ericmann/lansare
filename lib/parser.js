...
var E_NO_APP_NAME_OR_ID = new Err( 1, 'No application name or ID.' ),
	...
	E_NO_CONFIG = new Err( 16, 'No `.lansarerc` file in project.' ),
...
function Parser( program ) {
	...
	// First things first, let's figure out if there is a `.lansarerc` file.
	var config_file = path.resolve( '.lansarerc' );
	if ( fs.existsSync( config_file ) ) {
		var config = JSON.parse( fs.readFileSync( config_file, 'utf8' ) );

		// Make sure the .lansarerc file is configured properly
		if ( ! config.projects ) {
			error = error ^ E_NO_CONFIG.code;
		} else {

			// Make sure .lansarerc specifies at least one enviornment
			if ( program.args.length < 1 ) {
				...
				if ( 2 !== args.length ) {
					...
					try {
						...
						app_config.project = args[0];
						app_config.environment = args[1];
